...
class OpenAPIView:
    def __init__(self, parent):
        ...
        add_button = ttk.Button(button_frame, text="+", command=self.add_spec)
        add_button.pack(side=tk.LEFT, padx=5)

        remove_button = ttk.Button(button_frame, text="-", command=self.remove_spec)
        remove_button.pack(side=tk.LEFT)

        parent.add(self.frame, text="OpenAPI")

    def add_spec(self):
        top = Toplevel(self.frame)
        top.title("Add OpenAPI Spec")
